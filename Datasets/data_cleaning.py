...
import numpy as np
import ast
from models.sentiment_score import Sentiment 
...
class DataCleaner:
    ...
    def clean_columns(self,df):
        # extract year and decade BEFORE clean_columns drops release_date
    ...
    def rating_transform(self, df, combined_ratings):
        combined_ratings1=combined_ratings.copy()
        avg_ratings = (combined_ratings1.groupby('movieId', as_index=False)['rating'].mean().rename(columns={'rating': 'ave_rating'}))

        df=df.merge(avg_ratings, on='movieId', how='left')
        df[["sentiment_score","ave_rating"]] = df[["sentiment_score","ave_rating"]].fillna(0)
        return df
